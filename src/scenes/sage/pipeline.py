"""
SAGE end-to-end pipeline: collect → export → render → bbox → questions.

Called by run.py via src.sage.pipeline.run(args).
"""
...
def run(args) -> int:
    from src.scenes.sage.loader import list_scene_ids, get_layout_path
    ...
    try:
        scene_ids = list_scene_ids(args.num_scenes, seed=getattr(args, "seed", None))
    except (FileNotFoundError, RuntimeError) as e:
        print(f"Error (SAGE): {e}", file=sys.stderr)
        return 1
    if not scene_ids:
        print("No SAGE scenes to process.", file=sys.stderr)
        return 1
    if len(scene_ids) < args.num_scenes:
        print(
            f"Warning: requested {args.num_scenes} SAGE scene(s), found {len(scene_ids)}."
        )

    # Determine if Isaac Sim is needed
    export_needed = any(
        not os.path.isfile(os.path.join(output_root, scene_id, "scene.json"))
        for scene_id in scene_ids
    )
    render_needed = any(
        needs_render(os.path.join(output_root, scene_id, "images"))
        for scene_id in scene_ids
    )
    sim_needed = export_needed or render_needed
    ...
    if sim_needed:
        ...
        from src.scenes.sage.scene import (
            export_scene as _export,
            render_scene as _render,
        )

        export_scene = _export
        render_scene = _render
    else:
    ...
    try:
        for i, scene_id in enumerate(scene_ids):
            ...
            print(f"\n{'=' * 60}\nSAGE Scene {i + 1}/{total}: {scene_id}\n{'=' * 60}")

            should_export = not os.path.isfile(scene_json)
            should_render = needs_render(images_dir)

            # Download scene on demand if any sim step is needed
            layout_path = None
            if should_export or should_render:
                layout_path = get_layout_path(scene_id)
            ...
            if not should_export:
                print("  [export] skipped (scene.json exists)")
            elif export_scene is None:
                print(
                    "  [export] FAILED: Isaac Sim not started but scene.json missing",
                    file=sys.stderr,
                )
                continue
            else:
            ...
            if not should_render:
                print(f"  [render] skipped (images exist: {images_dir})")
            else:
                try:
                    render_scene(
                        layout_path,
            ...
            if getattr(args, "cleanup", False):
                ...
                for f in ["scene.json", "view_filter.json"]:
                    ...
                    if os.path.isfile(p):
                        os.remove(p)
                from src.scenes.sage.loader import get_scenes_dir

                src_dir = os.path.join(get_scenes_dir(), scene_id)
